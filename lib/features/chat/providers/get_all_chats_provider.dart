import 'dart:async';

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

import '/core/constants/firebase_collection_names.dart';
import '/core/constants/firebase_field_names.dart';
import '/features/chat/models/chatroom.dart';

/// This provider will get all the chatrooms from firebase in which
/// you're a member. It will return everything in the form of chatrooms
final getAllChatsProvider =
    StreamProvider.autoDispose<Iterable<Chatroom>>((ref) {
  final myUid = FirebaseAuth.instance.currentUser!.uid;
  final controller = StreamController<Iterable<Chatroom>>();

  final chatrooms =
      FirebaseFirestore.instance.collection(FirebaseCollectionNames.chatrooms);

  final sub = chatrooms
      .where(FirebaseFieldNames.members, arrayContains: myUid)
      .snapshots()
      .listen((snapshots) {
    final chats = snapshots.docs.map(
      (chatData) => Chatroom.fromMap(chatData.data()),
    );
    controller.sink.add(chats);
  });

  ref.onDispose(() {
    controller.close();
    sub.cancel();
  });

  return controller.stream;
});
